### Generar contraseñas con longitud determinada y caracteres aleatorios o caracteres personalizados
> Se puede guardar el diccionario de caracteres y establecerlo predeterminado

###### Sin librerias necesarias a instalar

###### Ejemplo de uso
> pass [longitud] [cantidad] [argumentos]
...
| dicc | Usar un diccopnaro personalizado `(-s para guardarlo)`|
| log | Muestra el contenido de el registro `(-c para limpiarlo)`|
| pass | Generar una nueva contraseña `(-s para guardar en el log)`|
...
try:
    while True:
        ...
        if(command[0].lower() in ['-c', '--clear', 'c', 'clear']):
            try:
                os.system('cls')
            except:
                os.system('clear')
            ...
            if(len(command) == 1):
                ...
                if(os.path.isfile(command[1])):
                    ...
                    try:
                        if(command[2].lower() in ['-s', '--save', 's', 'save']):
            ...
            if(len(command) == 1):
                ...
                try:
                    if(command[1].lower() in ['-c', '--clear', 'c', 'clear']):
                ...
                if(int(command[1]) < 0):
                    ...
                    if not(command[1].isnumeric()):
                        ...
                        for _ in range(num):
                            ...
                            print(password)
                            try:
                                if(command[3].lower() in ['-s', '--save', 's', 'save']):
